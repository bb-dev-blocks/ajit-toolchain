#!/usr/bin/env python3
"""Headless qemu-ajit run helper for CoRTOS examples."""
...
def _smp_and_mem_mib(project_dir: str) -> Tuple[int, int]:
  """AJIT threads → qemu -smp; TotalMemoryInKB → -m (MiB, at least 128)."""
  path = os.path.join(project_dir, consts.CONFIG_FILE_DEFAULT_NAME)
  cores, tpc, mem_kb = 1, 1, 0
  if os.path.isfile(path):
    with open(path) as f:
      data = yaml.safe_load(f) or {}
    cores = int(data.get("Cores") or 1)
    tpc = int(data.get("ThreadsPerCore") or 1)
    mem_kb = int(data.get("TotalMemoryInKB") or 0)
  smp = max(1, min(cores * tpc, 4))
  mem_mib = max(128, (mem_kb + 1023) // 1024)
  return smp, mem_mib
