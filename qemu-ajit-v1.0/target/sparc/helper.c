...
target_ulong helper_rdasr29(CPUSPARCState *env)
{
    CPUState *cs = env_cpu(env);
    target_ulong core = cs->cpu_index / THREADS_PER_CORE;
    target_ulong thread = cs->cpu_index % THREADS_PER_CORE;

    /* CoRTOS / AJIT C-model: 0x50520<core>0<thread> */
    return 0x50520000 | (core << 8) | thread;
}

#include "qemu/timer.h"

target_ulong helper_rdasr30(CPUSPARCState *env)
{
    int64_t now_ns = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL);

    uint64_t delta_ns = now_ns - env->asr30_boot_ns;

    return (delta_ns / 10) >> 32;
}

target_ulong helper_rdasr31(CPUSPARCState *env)
{
    int64_t now_ns = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL);

    uint64_t delta_ns = now_ns - env->asr30_boot_ns;

    return (delta_ns / 10) & ((1ULL << 32) - 1);
}
#endif
